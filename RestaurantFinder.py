...
with open("KB.pl", "w") as text_file: text_file.write(KB_Ray)
...
payment_options = ['cash', 'card', 'paytm']
...
def get_restaurant():
    ...
    ans = False
    X = pyswip.Variable()
    q = pyswip.Query(pick_restaurant(X))
    sol = []
    ans = ''
    while q.nextSolution():
        if X.value not in sol:
            sol.append(X.value)
            ans = ans + str(X.value) + ', '
    if not len(sol): ans = "Sorry, couldn't found any restaurant."
    Label(master, text= "You can get food from: " + ans).grid(row=17, column = 3)
